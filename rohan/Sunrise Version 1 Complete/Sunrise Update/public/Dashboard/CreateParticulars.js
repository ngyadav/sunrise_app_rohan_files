...
window.onload = function () {
    ...
    var firebaseref=firebase.database().ref().child("Particulars")
    return firebaseref.once('value').then(function(snapshot) {
       snapshot.forEach(function(child){
        var option = document.createElement('option');
        option.text = option.value = child.val();
        select.add(option, 0);
       });
       select.value="Select";
    });
...
addbtn.addEventListener('click',function(){
    ...
    var firebaseref=firebase.database().ref().child("Particulars");
        firebaseref.child(username.value.toUpperCase().trim()).set(String(username.value.toUpperCase().trim().toUpperCase()));
