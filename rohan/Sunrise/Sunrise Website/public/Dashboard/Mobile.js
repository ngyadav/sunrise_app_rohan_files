...
window.onload = function () {
    ...
    .orderByChild("Category").equalTo("Mobile")
...
$("#mimage1").change(function(){
    ...
    if(mname.options[mname.selectedIndex].value=="Select")
        {
            alert("Select Mobile Baner Name");
            mname.focus();
            return;
        }

    if($('#mimage1').get(0).files.length==0){
        alert("Add Mobile Baner Image");
        return;
    }
    if(!window.path4=='')
    {
    window.mtemp--;
     ...
     const ref = firebase.storage().ref("/Mobile/");
...
submitbtn1.addEventListener('click',function(){
    ...
    var firebaseref=firebase.database().ref().child("AppContent").child("Mobile").child("Baner").child(mname.options[mname.selectedIndex].value);
...
submitbtn2.addEventListener('click',function(){
    ...
    var firebaseref=firebase.database().ref().child("AppContent").child("Mobile").child(mname.options[mname.selectedIndex].value);
